...
function Test-Servers {
    try{
        ...
        ForEach($NorthstarServer in $server.NorthstarServers){
            ...
            $startupargsdedi = "$($NorthstarServer.AbsolutePath)\ns_startup_args_dedi.txt"
            $autoexecdedi = "+exec " + $autoexecserver
            Write-FileUtf8 -Append $False -InputVar "//Config file generated by PSNorthstarWatcher on $(get-date)" -Filepath $configfilepath
            Write-FileUtf8 -Append $False -InputVar "-multiple -dedicated $autoexecdedi" -Filepath $startupargsdedi
            Write-Host "Overwriting $configfilepath and $startupargsdedi"
